...
// runPool, rewriteDuration, DEFAULT_JOBS) rather than re-implementing it —
// only the staging SOURCE changes: pipelines/video/intro-kit/ instead of
// card-library/. The ~8 staging lines are duplicated with a comment naming
// the original, because lib/render.mjs's own staging block is inlined inside
// its single renderOne() closure and pulling it out would touch how the body
// pipeline renders every card — the plan's STOP condition on this exact point.
...
function renderCardBeat(beat, { renderDir, cacheDir, noCache, workdir }) {
  ...
  try {
    ...
    fs.writeFileSync(path.join(stagedDir, 'vars.json'), JSON.stringify(beat.vars ?? {}));
...
export async function renderSimple(slugOrWorkdir, { jobs = DEFAULT_JOBS, noCache = false } = {}) {
  ...
  await runPool(cardBeats, jobs, async (beat) => {
    renderedByBeat[beat.id] = renderCardBeat(beat, { renderDir, cacheDir, noCache });
  });
