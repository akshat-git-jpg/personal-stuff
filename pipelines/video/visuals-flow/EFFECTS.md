...
- **Drift — REMOVED** (2026-07-24, owner rule): the slow Ken Burns zoom on screen segments was judged "not adding much value — remove forever" in test-01 Final Cut review. `drift.mjs` was deleted; screen segments render static. Do not reintroduce without an explicit owner decision (guarded by the "screen segments stay static" integration test in `lib/assemble.test.mjs`).
