...
- **Next:** step 012 cleans the transcript; 015 maps the segments; 020 and 030 then read `transcript.json`

**Transcript quality pass.** `run.sh` always keeps the raw engine output as
`transcript.<engine>-raw.bak.json` first. See step 012 (`steps/012-clean-transcript-llm`)
for the actual cleanup procedure.
