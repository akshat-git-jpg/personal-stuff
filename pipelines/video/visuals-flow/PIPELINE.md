...
| `115-author-intro-simple-llm` | [LLM] | `transcript.json` + `segments.json` + `concept.json` + `../intro-kit/kit.json` → `intro-simple/cutlist.json`. Runs only when `run-config.json` has `introMode: "simple"`. Picks a card slug per beat from the locked 7-card kit and fills its variables — it never writes HTML. Approved at 125, rendered at 135. |
...
| `125-approve-intro-simple-human` | [OWNER] | `intro-simple/cutlist.json` → `approved: true` in `cutlist.json` (HUMAN GATE: Intro (simple)). Reviewed on the Intro tab of the board against the rendered `intro-film/out/intro.mp4` (135 runs before this gate despite the lower number — same reason 150/160 order this way in the complex flow; see steps/150-approve-intro-film-human/step.json's own `consumes`, which omits its render output for the identical reason). |
| `130-author-intro-screenplay-llm` | [LLM] | `transcript.json` + `segments.json` + `concept.json` + `card-library/DESIGN.md` → `intro-film/screenplay.json`. Runs only when `run-config.json` has `intro: "film"`. Reads the design system and the logo registry, and must not read `catalog.json` — the intro keeps full creative freedom. Reviewed at 140, approved at 150, rendered at 160. |
| `135-render-intro-simple-run` | [RUN] | `intro-simple/cutlist.json` → `intro-film/out/intro.mp4`. Refuses to run if the cut list fails the S1-S7 pacing lint. No approval check here on purpose, mirroring the complex flow's 160 — rendering is how the owner GETS a cut to watch; the gate (125) is enforced downstream at assembly (`requireIntroApproved()`). |
...
| `445-rerender-intro-simple-run` | [RUN] | the real `avatar.mp4` + the approved cut list -> a final `intro-film/out/intro.mp4`. The intro approved at 125 was cut against a static stand-in; this is the encode that ships. Fails if the workdir still has no real avatar. Mirrors 440 for the complex flow. |
