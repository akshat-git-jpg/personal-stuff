...
grep -q 'bash steps/080-storyboard-review-owner/run.sh "$slug"' run.sh || fail "missing board command"
grep -q 'bash steps/090-render-run/run.sh "$slug"' run.sh || fail "missing render command"
...
grep -q 'bash steps/100-avatar-render-run/run.sh "$slug"' run.sh || fail "missing avatar command"
grep -q 'bash steps/110-assemble-run/run.sh "$slug"' run.sh || fail "missing assemble command"
grep -q 'bash steps/140-resolve-export-run/run.sh "$slug"' run.sh || fail "missing export command"
grep -q 'bash scripts/qc-video.sh "$slug"' run.sh || fail "missing qc command"
grep -q 'bash steps/110-assemble-run/run.sh "$slug" --draft' run.sh || fail "missing cut command"
