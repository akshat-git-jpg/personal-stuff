...
This step exists inside `visuals-flow-2` precisely so it does NOT re-derive
...
## Taste
Read `TASTE-INTRO.md` at the visuals-flow-2 root. It is the accumulated record of what the owner has rejected on screen, as numbered rules. Some are enforced by the review pass; the rest will only be caught by you reading them.
