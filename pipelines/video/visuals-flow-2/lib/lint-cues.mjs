...
export function lintCues({ cuesFile, resolved, words, catalog, segmentsData, manifest, conceptData, avatarJobs = null }) {
  ...
  // W21 long-hold (owner fold 2026-07-31, opusclip-vs-submagic z05): a cue held
  ...
  for (const r of sortedResolved) {
    ...
    if (r.duration > 2 * cat.default_duration) {
      warnings.push(`W21 long-hold: ${r.id} holds ${r.card} for ${r.duration.toFixed(1)}s, ${(r.duration / cat.default_duration).toFixed(1)}x its designed ${cat.default_duration}s — confirm the card's hold loop is visible for the tail (DESIGN.md long-hold rule)`);
    }
  }
