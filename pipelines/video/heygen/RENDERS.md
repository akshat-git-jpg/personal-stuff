...
**Media files live outside the repo** in `~/kb-scratch/video/heygen/<pipeline>/` — `_test/`
for renders tied to no pipeline (media policy); this tracked manifest is the record. Rows in
"Videos generated" are auto-appended on submit by heygen-web's `src/cli/render-log.mjs`
...
relative to `~/kb-scratch/video/heygen/_test/`.
