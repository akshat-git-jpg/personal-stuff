...
> **Moved 2026-07-12** from `pipelines/youtube/open-source/avatar/` into the heygen asset
> hub. The scripts + pose images now live in this folder; the generated test media
> (`*.mp4`, `audio-25s.wav`) moved to `~/kb-scratch/video/heygen/_test/fal-lipsync/`
> (media policy). Mentions of `test/` paths below refer to those two locations.
