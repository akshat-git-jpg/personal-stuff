/**
 * BakeoffC — "PRESENCE IN THE WORLD" (dark register + matted speaker).
 * The human is the anchor and is moved THROUGH the world: hero on the hook, steps aside as the
 * stream reveals, dims to the edge on the burst (graphics take over), corner-PIP for photo-vs-movie,
 * returns HERO for the direct-address aside, steps aside for the unanswered order, closes on the line.
 * Speaker = rembg PNG cutout (public/cutout_bk, source 25fps). Voice only; music+SFX mixed in post.
...
const Card: React.FC<{ x: number; y: number; u: number; label: string; money?: boolean; scale?: number; rot?: number; opacity?: number; dark?: boolean }> = ({ x, y, u, label, money, scale = 1, rot = 0, opacity = 1, dark }) => (
  <div style={{ position: "absolute", left: x + "%", top: y + "%", transform: `translate(-50%,-50%) rotate(${rot}deg) scale(${scale})`, opacity }}>
    <div style={{ position: "relative", display: "inline-block" }}>
      ...
      <div style={{ position: "relative", background: dark ? RAISIN_DEEP : WHITE, border: `${u * 0.11}px solid ${dark ? LIME : RAISIN}`, borderRadius: u * 0.5, padding: `${u * 0.4}px ${u * 0.8}px`, display: "flex", alignItems: "center", gap: u * 0.5, boxShadow: `0 ${u * 0.6}px ${u * 1.4}px rgba(0,0,0,0.5)` }}>
        <span style={{ width: u * 0.5, height: u * 0.5, background: money ? LIME : "transparent", border: `${u * 0.1}px solid ${dark ? LIME : RAISIN}`, flexShrink: 0 }} />
        <span style={{ fontFamily: SANS, fontWeight: 600, fontSize: u * 1.1, color: dark ? SILVER : RAISIN, whiteSpace: "nowrap" }}>{label}</span>
      </div>
    </div>
  </div>
);

/* ---- the speaker layer (ground shadow + lime backlight + matte) ---- */
const Speaker: React.FC<{ t: number; sk: SK; u: number }> = ({ t, sk, u }) => {
  const idx = Math.max(0, Math.min(SRC_MAX, Math.round(t * SRC_FPS)));
  ...
  return (
    <div style={{ position: "absolute", left: sk.x + "%", top: sk.y + "%", transform: "translate(-50%,-50%)", pointerEvents: "none" }}>
      {/* lime backlight */}
      <div style={{ position: "absolute", left: "50%", top: "42%", width: w * 0.9, height: w * 0.9, transform: "translate(-50%,-50%)", background: `radial-gradient(circle, ${LIME}22 0%, transparent 62%)`, opacity: bright }} />
      {/* ground shadow */}
      <div style={{ position: "absolute", left: "50%", top: `calc(50% + ${w * 0.28}px)`, width: w * 0.8, height: w * 0.12, transform: "translate(-50%,-50%)", background: "radial-gradient(ellipse, rgba(0,0,0,0.55) 0%, transparent 70%)" }} />
      <Img src={staticFile(`cutout_bk/frame_${pad5(idx)}.png`)} style={{ width: w, display: "block", filter: `brightness(${0.62 + 0.38 * bright}) contrast(1.03) drop-shadow(0 ${u * 0.8}px ${u * 1.2}px rgba(0,0,0,0.6))` }} />
    </div>
  );
};

export const BakeoffC: React.FC = () => {
  ...
  /* speaker path through the world */
  const SKF: SK[] = [
    { t: 0.0, x: 55, y: 55, s: 1.16, dim: 0 },
    { t: 2.3, x: 55, y: 55, s: 1.16, dim: 0 },
    { t: 3.3, x: 75, y: 55, s: 1.0, dim: 0.05 },   // step aside → stream reveals left
    { t: 6.0, x: 75, y: 55, s: 1.0, dim: 0.05 },
    { t: 6.5, x: 78, y: 56, s: 0.95, dim: 0.15 },  // watch the file
    { t: 9.1, x: 78, y: 56, s: 0.95, dim: 0.18 },
    { t: 9.7, x: 90, y: 58, s: 0.86, dim: 0.5 },   // BURST — graphics take over, dim to edge
    { t: 11.0, x: 90, y: 58, s: 0.86, dim: 0.5 },
    { t: 11.9, x: 86, y: 68, s: 0.58, dim: 0.12 }, // movie: corner PIP
    { t: 15.0, x: 86, y: 68, s: 0.58, dim: 0.12 },
    { t: 15.7, x: 50, y: 54, s: 1.2, dim: 0 },     // ASIDE — hero, direct address
    { t: 21.0, x: 50, y: 54, s: 1.2, dim: 0 },
    { t: 22.1, x: 73, y: 55, s: 1.0, dim: 0.05 },  // step aside → unanswered order
    { t: 24.8, x: 73, y: 55, s: 1.0, dim: 0.05 },
    { t: 25.5, x: 60, y: 55, s: 1.1, dim: 0 },     // close on the line
    { t: 27.8, x: 60, y: 55, s: 1.1, dim: 0 },
  ];
  const sk = skf(SKF, frame);

  /* stream (behind speaker), lower-left band */
  const streamOn = clamp01((t - 2.6) / 0.8) * (1 - clamp01((t - 10.0) / 0.8)); // stream lives only in its beat
  ...
  const TRAVERSE = 6.5;
  const ridersClear = 1 - clamp01((t - 10.0) / 0.6); // hard-clear all riders as we leave the stream zone
  const streamY = (p: number) => 74 - p * 8 + Math.sin(p * 7) * 2;   // % , rising left→right
  const streamX = (p: number) => -6 + p * 62;                        // % , into left-center
  ...
  return (
    <AbsoluteFill style={{ background: RAISIN, overflow: "hidden" }}>
      ...
      {/* atmosphere */}
      <AbsoluteFill style={{ background: `radial-gradient(ellipse 70% 60% at 42% 46%, ${RAISIN_DEEP} 0%, ${RAISIN} 68%)` }} />
      <AbsoluteFill style={{ backgroundImage: `linear-gradient(${SILVER}0a 1px,transparent 1px),linear-gradient(90deg,${SILVER}0a 1px,transparent 1px)`, backgroundSize: `${u * 3.4}px ${u * 3.4}px`, maskImage: "radial-gradient(ellipse 80% 70% at 45% 55%, #000 30%, transparent 85%)", WebkitMaskImage: "radial-gradient(ellipse 80% 70% at 45% 55%, #000 30%, transparent 85%)" }} />

      {/* ===== world (shake on burst) ===== */}
      <div style={{ position: "absolute", inset: 0, transform: `translate(${shake}px,${shake * 0.5}px)` }}>
        {/* stream wire + riders (BEHIND speaker) */}
        <svg width={W} height={H} style={{ position: "absolute", inset: 0 }}>
          <defs>
            <linearGradient id="wC" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0" stopColor={LIME} stopOpacity="0" /><stop offset="0.2" stopColor={LIME} stopOpacity="0.9" /><stop offset="1" stopColor={LIME} stopOpacity="0.9" />
            </linearGradient>
          </defs>
          {streamOn > 0 && (
            <path d={(() => { let d = `M ${streamX(0) / 100 * W} ${streamY(0) / 100 * H}`; for (let p = 0.02; p <= 1; p += 0.02) d += ` L ${streamX(p) / 100 * W} ${streamY(p) / 100 * H}`; return d; })()}
              stroke="url(#wC)" strokeWidth={u * 0.32} fill="none" strokeLinecap="round" strokeDasharray={`${u * 2.2} ${u * 1.3}`} strokeDashoffset={-t * u * 7}
              opacity={streamOn} style={{ filter: `drop-shadow(0 0 ${u * 0.7}px ${LIME}aa)` }} />
          )}
        </svg>
        {riders.map((r, i) => {
          const p = (t - r.born) / TRAVERSE; if (p <= 0 || p >= 0.78) return null;
          const x = streamX(p), y = streamY(p);
          if (x > 46) return null;
          // narrow visible window so only ~3 cards are on the stream at once (no clumping)
          const vis = clamp01((p - 0.06) / 0.1) * (1 - clamp01((p - 0.6) / 0.15)) * ridersClear;
          if (vis <= 0) return null;
          return <Card key={`r${i}`} x={x} y={y + amb.bob(i * 0.4, 0.5)} u={u} label={r.label} money={r.money} scale={0.66 + p * 0.28} rot={amb.drift(i * 0.5, 2.4)} opacity={vis} dark />;
        })}
        ...
        {impacts.map((it, k) => {
          ...
          return <Card key={`d${k}`} x={bz(sx, mx, ex)} y={bz(sy, my, ey)} u={u} label={LABELS[(k * 3 + 1) % LABELS.length]} money={k % 2 === 0} scale={0.95 - p * 0.4} rot={p * -12} dark />;
        })}
        {burstP > 0 && DEBRIS.map((d, i) => { const dt = t - 9.6; if (dt > 1.3) return null;
          ...
          return <Card key={`b${i}`} x={x} y={y} u={u} label={LABELS[i % LABELS.length]} money={i % 2 === 0} scale={0.6} rot={d.rot * dt} opacity={1 - dt / 1.3} dark />; })}
      </div>

      {/* ============ SPEAKER ============ */}
      <Speaker t={t} sk={sk} u={u} />

      {/* ============ FOREGROUND (over speaker) ============ */}
      {/* headline: NOT A DOCUMENT (hero beat, upper-left) */}
      {t > 0.8 && t < 5.6 && (
        <div style={{ position: "absolute", left: "6%", top: "16%", whiteSpace: "nowrap", fontFamily: SANS, fontWeight: 800, fontSize: u * 3.7, letterSpacing: "-0.02em", color: SILVER, textTransform: "uppercase", opacity: clamp01((t - 0.8) / 0.4) * (1 - clamp01((t - 5.2) / 0.4)) }}>
          NOT A DOCUMENT.
        </div>
      )}
      {t > 3.0 && t < 9.4 && (
        <div style={{ position: "absolute", left: "6%", top: "23.5%", whiteSpace: "nowrap", fontFamily: SANS, fontWeight: 800, fontSize: u * 3.7, letterSpacing: "-0.02em", color: SILVER, textTransform: "uppercase", opacity: clamp01((t - 3.0) / 0.4) * (1 - clamp01((t - 9.0) / 0.4)) }}>
          IT'S A {marker("STREAM.", 3.9)}
        </div>
      )}
      ...
      {/* photo vs movie (center, speaker is corner-PIP) */}
          ...
          <div style={{ position: "absolute", inset: 0, opacity: on }}>
            ...
            <div style={{ position: "absolute", left: "27%", top: "44%", transform: "translate(-50%,-50%)" }}>
              <div style={{ width: u * 20, height: u * 15, background: "#14171f", border: `${u * 0.14}px solid ${SILVER_MID}`, position: "relative" }}>
                {[0, 1, 2].map((i) => <Card key={i} x={30 + i * 20} y={35 + (i % 2) * 26} u={u * 0.62} label={["order", "DM", "€90"][i]} money={i !== 1} scale={0.5} dark />)}
              </div>
            ...
            <div style={{ position: "absolute", left: "55%", top: "44%", transform: "translate(-50%,-50%)" }}>
              <div style={{ width: u * 22, height: u * 15, background: "#14171f", border: `${u * 0.16}px solid ${LIME}`, position: "relative", overflow: "hidden", boxShadow: `0 0 ${u * 2}px ${LIME}55` }}>
                {[0, 1, 2].map((i) => { const lx = 8 + ((t * 20 + i * 34) % 92); return <Card key={i} x={lx} y={32 + (i % 2) * 30 + amb.bob(i, 6)} u={u * 0.62} label={["order", "DM", "€90"][i]} money={i !== 1} scale={0.5} dark />; })}
              </div>
      ...
      {/* comment card (hero aside) */}
      {t > 15.9 && t < 21.4 && (() => { const on = clamp01((t - 16.0) / 0.5) * (1 - clamp01((t - 21.0) / 0.4));
        return (
          <div style={{ position: "absolute", left: "27%", top: "72%", transform: "translate(-50%,-50%)", opacity: on }}>
            <div style={{ position: "relative", display: "inline-block" }}>
              <div style={{ position: "absolute", inset: 0, background: LIME, borderRadius: u * 0.8, transform: `translate(${u * 0.5}px,${u * 0.5}px)` }} />
              <div style={{ position: "relative", background: WHITE, border: `${u * 0.12}px solid ${RAISIN}`, borderRadius: u * 0.8, padding: `${u * 0.9}px ${u * 1.3}px`, display: "flex", alignItems: "center", gap: u * 0.8, width: u * 34 }}>
      ...
      {/* unanswered new order */}
      {t > 22.0 && t < 25.2 && (() => { const dt = t - 22.2; const drop = clamp01(dt / 0.5); const bounce = dt > 0.5 && dt < 0.8 ? -Math.sin((dt - 0.5) / 0.3 * Math.PI) * 2 : 0;
        return (<>
          <Card x={31} y={30 + drop * drop * 22 + bounce} u={u} label="new order €1,850" money scale={1.05} rot={-3} opacity={clamp01(dt / 0.3)} dark />
