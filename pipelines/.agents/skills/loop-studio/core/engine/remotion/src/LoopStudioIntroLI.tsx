/**
 * LoopStudioIntroLI — the masterclass intro in the Business Brain visual language.
...
const FPS = 30;
const SRC = "loopstudio/cut_li.mp4";
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <div style={{ position: "relative", width: VW, height: VH, borderRadius: u * 0.5, overflow: "hidden", boxShadow: `0 ${u * 1.2}px ${u * 3.4}px rgba(0,0,0,0.55)` }}>
        <OffthreadVideo src={staticFile("loopstudio/cut_li.mp4")} startFrom={330} muted style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", filter: "contrast(1.08) saturate(1.12) brightness(1.03)" }} />
...
export const LoopStudioIntroLI: React.FC = () => {
  const frame = useCurrentFrame();
  const tR = frame / FPS;   // LinkedIn real time
  ...
  const SKF: SK[] = [
    { t: 0, x: 50, y: 50, s: 1, dim: 1, fr: 1 },            // TURN: head hidden — the TV room
    { t: 3.9, x: 50, y: 50, s: 1, dim: 1, fr: 1 },
    { t: 4.1, x: 73, y: 52, s: 0.94, dim: 0.06, fr: 1 },    // SCOPE: head returns to panel
    { t: 7.7, x: 73, y: 52, s: 0.94, dim: 0.06, fr: 1 },
    { t: 8.3, x: 50, y: 50, s: 1, dim: 1, fr: 1 },          // MECH + COST + CRAZY hidden
    { t: 21.7, x: 50, y: 50, s: 1, dim: 1, fr: 1 },
  ];
  ...
  const liteWins: [number, number][] = [[4.0, 14.1]];
  const lite = Math.max(...liteWins.map(([a, b]) => clamp01((tR - a) / 0.5) * (1 - clamp01((tR - b) / 0.5))), 0);
  ...
  return (
    <AbsoluteFill style={{ background: RAISIN, overflow: "hidden", fontFamily: SANS }}>
      <Audio src={staticFile("loopstudio/vo_li.m4a")} />
      ...
      <div style={{ position: "absolute", inset: 0, opacity: graphicsOn }}>
        ...
        {tR >= 0.0 && tR < 4.35 && (() => {
          const t = tR + 6.36;   // remap to the scene's original time
        ...
        {tR >= 4.05 && tR < 8.5 && (() => {
          const t = tR + 6.4;   // remap to the scene's original time
        ...
        {tR >= 7.85 && tR < 14.15 && (() => {
          const t = tR + 8.22;   // remap to the scene's original time
        ...
        {tR >= 14.4 && tR < 19.6 && (() => {
          const t = tR + 7.9;   // remap to the scene's original time
        ...
        {tR >= 18.95 && tR < 21.7 && (() => {
          const t = tR + 14.42;   // remap to the scene's original time
      ...
      <Sfx n="whoosh" at={0.3} v={0.18} dur={1.4} />          {/* TV turns on */}
      <Sfx n="ping" at={7.4} v={0.2} dur={1.0} />             {/* EDITED */}
      <Sfx n="whoosh" at={10.3} v={0.22} dur={1.0} />         {/* editor dismissed */}
      <Sfx n="complete" at={13.1} v={0.24} dur={1.4} />       {/* note done */}
      <Sfx n="whoosh" at={14.0} v={0.16} dur={1.2} />         {/* crossfade to dark */}
      <Sfx n="complete" at={17.4} v={0.26} dur={1.4} />       {/* $0 */}
      <Sfx n="riser" at={19.3} v={0.14} dur={2.2} />          {/* the crazy climb */}
    </AbsoluteFill>
  );
};
...
export default LoopStudioIntroLI;
