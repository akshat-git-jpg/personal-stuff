#!/usr/bin/env python3
"""
Step 120 — make timestamped transcript from the FINAL voiceover.  [RUN]

  python3 run.py [<voice.trim.wav>]         # default input: step 100's output
...
editor-ready artifacts mapped to the REAL audio, so an editor can scrub straight to any line:
  ...
  output/<base>.timestamps.json raw Groq segments + words (input to step 130, plan-visuals)
...
OUT = HERE / "output"
PREV = ROOT / "4-voiceover/100-trim-silence-run/output"
...
def main():
    ...
    ap.add_argument("wav", nargs="?", help="final voiceover (default: step 100's output)")
    a = ap.parse_args()

    wav = pathlib.Path(a.wav) if a.wav else next(iter(sorted(PREV.glob("*.trim.wav"))), None)
    if not wav or not wav.exists():
        die(f"no voiceover found (pass a path, or run step 100 first → {PREV})")
    ...
    print("→ next: step 130 (plan-visuals) — Claude applies its rulebook to these timestamps")
