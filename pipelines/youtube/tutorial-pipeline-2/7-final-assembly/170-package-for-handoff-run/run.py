...
OUT = HERE / "output"
S05_OUT = ROOT / "2-recording/010-create-drive-folders-run/output"
...
def infer_base(arg):
    if arg:
        return arg
    cands = sorted((ROOT / "4-voiceover/100-trim-silence-run/output").glob("*.voice.trim.wav"))
    if not cands:
...
def load_manifest(title, title_arg):
    ...
    if not mpath.exists():
        ...
        die(f"no Drive folders for {title!r} — run step 010 first:\n"
            f"  python3 ../../2-recording/010-create-drive-folders-run/run.py --title {title_arg!r}\n"
...
def main():
    ...
    copy_glob(ROOT / "6-avatar-clips/160-download-avatar-videos-human/output/videos", f"{base}__a4__*.mp4",
              ve_in / "full-block-spokesperson", log, "A4 full-screen videos")
    copy_glob(ROOT / "6-avatar-clips/160-download-avatar-videos-human/output/videos", f"{base}__a3__*.mp4",
              ve_in / "talking-head-spokesperson", log, "A3 corner videos")
    # plan + reference docs
    plan = ve_in / "plan"; plan.mkdir(parents=True, exist_ok=True)
    for src, label in [
        (ROOT / f"5-visuals/130-plan-visuals-sonnet/output/{base}.visual-plan.md", "visual plan"),
        (ROOT / f"4-voiceover/080-synthesize-voice-run/output/{base}.avatar-fullscreen.md", "avatar script"),
        (ROOT / f"4-voiceover/120-make-timestamped-transcript-run/output/{base}.srt", "subtitles (srt)"),
        (ROOT / f"4-voiceover/120-make-timestamped-transcript-run/output/{base}.timestamps.txt", "timestamped transcript"),
    ]:
        if src.exists():
    ...
    # the full voiceover
    copy_glob(ROOT / "4-voiceover/100-trim-silence-run/output", f"{base}.voice.trim.wav", ve_in / "audio", log, "voiceover (full)")
