#!/usr/bin/env python3
"""
Step 020 — trim dead air from the voiceover.  [RUN]

  python3 run.py [<voice.wav>] [--floor -40] [--edge 0.15] [--tighten] [--max-pause 0.7]

Default input: step 010's `output/<base>.voice.wav`. Always trims leading/trailing silence.
...
def main():
    ...
    ap.add_argument("wav", nargs="?", help="voiceover to trim (default: step 010's output)")
    ...
    if not src or not src.exists():
        die(f"no voiceover found (pass a path, or run step 010 first → {PREV})")
    ...
    print("→ next: listen; then step 050 (make-timestamped-transcript)")
