...
OUT = HERE / "output"
S = ROOT / "steps"
S05_OUT = S / "010-create-drive-folders/output"
...
def infer_base(arg):
    if arg:
        return arg
    cands = sorted((S / "100-trim-silence/output").glob("*.voice.trim.wav"))
    if not cands:
...
def load_manifest(title, title_arg):
    ...
    if not mpath.exists():
        ...
        die(f"no Drive folders for {title!r} — run step 010 first:\n"
            f"  python3 ../010-create-drive-folders/run.py --title {title_arg!r}\n"
...
def main():
    ...
    for src, label in [
        (S / f"130-plan-visuals/output/{base}.visual-plan.md", "visual plan"),
        (S / f"080-synthesize-voice/output/{base}.avatar-fullscreen.md", "avatar script"),
        (S / f"120-make-timestamped-transcript/output/{base}.srt", "subtitles (srt)"),
        (S / f"120-make-timestamped-transcript/output/{base}.timestamps.txt", "timestamped transcript"),
    ]:
        if src.exists():
    ...
    # the full voiceover
    copy_glob(S / "100-trim-silence/output", f"{base}.voice.trim.wav", ve_in / "audio", log, "voiceover (full)")
