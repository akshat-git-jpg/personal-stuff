...
- **Browser UIs are component apps (root decisions.md 2026-07-31).** A pipeline tool's web UI with more than one view, saving state, or real growth is a Vite+React+TS app like `video/visuals-flow-2/board-ui/` (own `package.json`, `dist/` gitignored, served by its owning server) — never a new hand-rolled HTML-template-string server. Trivial single-view static reports are the only exception.
