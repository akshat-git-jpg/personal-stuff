...
- **After any change:** run `./relink.sh`, then **restart** the affected session — skill
  discovery is cached, so `/skills` (and `claude -p`) won't reflect changes until relaunch.
- **Don't move/rename this folder** without rerunning `relink.sh` — symlinks would dangle.
