...
description: Opinionated production layer ON TOP OF the official Hyperframes skills — a three-level workflow, 16 lint gotchas with fixes, 10 copy-paste motion-graphics recipes, composition + storyboard templates, and a talking-head cut pipeline (silence-cut → whisper transcribe → retake-cut). Use for the production WORKFLOW, recipes, and lint-error fixes — for basic Hyperframes authoring/rendering use the official `hyperframes` skill instead. Triggers on "/hyperframes-helper", "hyperframes helper", "storyboard a video", "storyboard the title cards", "level 3 video pipeline", "cut the retakes", "silence cut my recording", "hyperframes lint gotchas", "motion graphics recipe".
user-invocable: true
metadata:
