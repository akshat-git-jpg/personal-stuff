...
| 268 | **`coffee` — keep the Mac awake with the lid shut, bounded** so a long job survives the laptop going in a bag. A launchd watchdog re-asserts normal sleep every 20s unless a live, unexpired lease says otherwise; four safety nets (deadline, 20% battery floor, watchdog-dies, no valid lease) mean there is no way to leave it on by accident. Executed INLINE by the orchestrator at the owner's explicit request, not dispatched | P2 | M | none | DONE |
