...
- `pipelines/video/visuals-flow-2/lib/resolve.mjs` line 37 exports `validateVariable`.
- `pipelines/video/visuals-flow-2/EDITOR-STYLE-GUIDE.md` exists, and its palette is **identical** to v1's:
  ```
  diff <(grep -o '#[0-9A-Fa-f]\{6\}' visuals-flow/EDITOR-STYLE-GUIDE.md | sort -u) \
       <(grep -o '#[0-9A-Fa-f]\{6\}' visuals-flow-2/EDITOR-STYLE-GUIDE.md | sort -u)
  ```
...
| Find v1 refs | `git grep -n "visuals-flow/" -- . ':(exclude)pipelines/video/visuals-flow/*' \| grep -v "visuals-flow-2/"` | see Step 5 |
...
- v2 itself. No behaviour change anywhere in `visuals-flow-2`.
...
cd pipelines/video
diff <(grep -o '#[0-9A-Fa-f]\{6\}' visuals-flow/EDITOR-STYLE-GUIDE.md | sort -u) \
     <(grep -o '#[0-9A-Fa-f]\{6\}' visuals-flow-2/EDITOR-STYLE-GUIDE.md | sort -u) && echo IDENTICAL
```
...
In `check-catalog.mjs`, change the import to `'../../visuals-flow-2/lib/resolve.mjs'`.
In `check-cards.sh`, change the path to `../visuals-flow-2/EDITOR-STYLE-GUIDE.md`.
...
**Verify**: `ls pipelines/video/ | grep -c "^visuals-flow$"` -> `0`, and `ls pipelines/video/ | grep -c "^visuals-flow-2$"` -> `1`

### Step 4: Update the routing docs

- `pipelines/CLAUDE.md`: delete the `video/visuals-flow/` map row. Leave the `visuals-flow-2` row.
- `pipelines/video/card-library/DESIGN.md` line 3 and `README.md` line 8: repoint `../visuals-flow/` to `../visuals-flow-2/`.
- `tutorial-pipeline-3` `PIPELINE.md` + `steps/070-handoff-visuals/README.md`: handoff artifacts now target `pipelines/video/visuals-flow-2/videos/<slug>/`.
- `tutorial-pipeline-2` rulebook line 16: repoint to `pipelines/video/visuals-flow-2/steps/020-cue-pass-llm/RULEBOOK.md`.

**Verify**: `cd /Users/kbtg/codebase/personal-stuff && git grep -n "visuals-flow/" -- pipelines ':(exclude)pipelines/video/visuals-flow/*' | grep -v "visuals-flow-2/" | grep -v loop-studio` -> no output

### Step 5: Confirm only historical records mention v1

```bash
cd /Users/kbtg/codebase/personal-stuff
git grep -l "visuals-flow/" -- . ':(exclude)pipelines/video/visuals-flow/*' | grep -v "visuals-flow-2"
```
...
- [ ] `git diff --stat` shows NO change under `pipelines/video/visuals-flow-2/`
...
- Deleting v1 requires any change inside `visuals-flow-2/`. It should not; report what forced it.
- `git status` shows deletions under `pipelines/video/visuals-flow-2/videos/`. You deleted the wrong folder — stop immediately.
