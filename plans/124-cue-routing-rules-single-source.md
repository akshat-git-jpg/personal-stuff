...
ui:
deploy:
needs: ["Nothing blocks this. It blocks 125. 126, 127 and 128 already landed."]
---
...
1. `EDITOR-STYLE-GUIDE.md` taught the PRE-2026-07-21 density philosophy ("The default is NO graphic", "one fullframe beat every 45 to 90 seconds", "If in doubt during demo stretches, skip the graphic") for months after root `decisions.md:157` reversed that stance. Nobody updated the guide because nothing forced them to. Plan 128 deleted the section on 2026-07-22 and replaced it with a pointer, so that copy is gone — but only by hand, and the same hand-copy could be written again tomorrow. This plan removes the reason anyone would.
