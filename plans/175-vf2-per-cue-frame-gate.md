...
- **Goals**:
  - A per-cue frame gate that probes every resolved cue's card with its REAL variables at its real beat times, headlessly, and FAILS `run.sh <slug> resolve` (step 040) on overflow/truncation (new lint code E13).
...
- **Done criteria** (terse): `bash scripts/check.sh` green including new tests; E13 fires on a knowingly-overflowing fixture; accent check fails on a frame with no accent pixels; both pass on the real opusclip-vs-submagic workdir.
- **Stop conditions** (terse): never weaken/skip an assertion to pass; STOP if hyperframes renders in tests exceed 90s each or Chrome cannot launch; STOP if E13 flags >5 cues on opusclip-vs-submagic (that means the probe policy is wrong, not the cards).
...
> `git diff --stat 8ad09a76..HEAD -- pipelines/video/visuals-flow-2/lib pipelines/video/visuals-flow-2/run.sh pipelines/video/visuals-flow-2/scripts pipelines/video/card-library/scripts/overflow-probe.mjs`
> If `lib/resolve.mjs`, `lib/render.mjs`, `run.sh`, or `overflow-probe.mjs` changed since 8ad09a76 in ways that contradict the excerpts below, STOP and report.
...
## Current state (facts, verified 2026-07-31 at 8ad09a76)
...
- `bash run.sh opusclip-vs-submagic resolve` — 040 on the real workdir. Expected after this plan: exits 0, warnings only, E13 absent.
...
- `pipelines/video/visuals-flow-2/lib/resolve.mjs` (wire E13)
...
// Returns array of error strings (E13 …), empty when clean.
export async function frameGate(resolved, cardLibraryRoot, { only } = {}) {
  ...
  try {
    for (const cue of resolved) {
      ...
      if (res.broken) {
        errors.push(
          `E13 frame-gate: ${cue.id} (${cue.card}) overflows the canvas at t=${res.t}s ` +
...
If the enclosing function is not async, make the minimal call-chain async — `run.sh resolve` invokes node once, so an async main is safe. E13 strings join the same error channel that makes `run.sh <slug> resolve` exit non-zero.

**Verify (must FAIL first — prove the gate can bite):** in a scratch copy of the workdir cues, set `videos/opusclip-vs-submagic/cues.json` c04's `variables.title` to a 14-word string, run `bash run.sh opusclip-vs-submagic resolve`; expect exit non-zero with an `E13 frame-gate: c04 …` line. Restore the file (`git checkout -- videos/opusclip-vs-submagic/cues.json`) — a leftover mutation is a STOP.

**Verify (real content passes):** `bash run.sh opusclip-vs-submagic resolve` exits 0 (warnings allowed, no `E13` lines).
...
- `test('frameGate flags an overflowing cue and passes a clean one', …)` — build two fake resolved cues against real card-library cards: clean = `slate/kinetic-sentence` with a 5-word text and its word beats; overflowing = `pros-cons/pros-cons` with `variables.title` of 16 words and beats `[ { kind:'pro', text:'x', at:1 } ]`. Assert exactly one E13 mentioning the overflowing id.
...
- Append one dated line to `tests/TESTS.md` under **Folded lessons**: `- 2026-07-31 — plan 175: the overflow probe became a per-cue GATE (E13 at 040, real variables at real beat times) and 090 gained the accent-visibility frame check (render-only color loss class). Badge → gate, per owner RCA 2026-07-30.`
...
3. Step 2's can-fail experiment produced an `E13 frame-gate: c04` error before the file was restored (paste the line into the run log), and `git status --porcelain videos/` is empty afterward.
4. `bash run.sh opusclip-vs-submagic resolve` → exit 0, no E13 lines.
...
- E13 flags more than 5 cues on the untouched opusclip-vs-submagic workdir → the policy (times/tolerance) is mis-tuned; STOP rather than loosening TOL.
