...
- Dayboard (Google Calendar day view) — https://dayboard.agrolloo.com (password-gated: APP_PASSWORD/SESSION_SECRET); READ-ONLY mirror of Google Calendar — no writes, no store. Worker + KV (`CACHE_KV`, 50-min Google token + 60s day cache only), static `public/`; app at `apps/dayboard`
