...
- Trip Map (trip pins + day plan + booking docs) — https://trips.agrolloo.com (no login; writes need `ADMIN_TOKEN` via `pp-trip`); Worker + KV (TRIPS_KV); app at `apps/trip-planner` (still on branch `work/trip-planner`, not landed)
