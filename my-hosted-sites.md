...
- Trip Planner (per-trip pin map: stay/transport/beach/sight/food/utility beside live location; navigation, live traffic and "near me" hand off to the Google Maps app) — https://trips.agrolloo.com (unlisted); installable PWA; Worker + KV (`TRIPS_KV`); `?trip=<slug>` deep-links to one trip; managed by `pp-trip` CLI + skill; app at `apps/trip-planner`
