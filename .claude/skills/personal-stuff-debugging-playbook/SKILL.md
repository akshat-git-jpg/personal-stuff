...
| 3 | Claude mobile / Remote Control: 401 on every message | `ssh root@72.61.241.170 'export PATH=/root/.local/bin:$PATH; claude auth status'` | `claude auth login` on the VPS (**NOT `setup-token`** — inference-only, Remote Control rejects it), restart `claude-rc.service`, start a NEW mobile session | Real incident, pre-2026-07-05, owner-recorded; no in-repo incident log — this row is the record (verified 2026-07-12). The auth check + expected output (`kushalbakliwal25@gmail.com`, `subscriptionType: pro`) are documented in `VPS-CRONS.md` health checks |
...
| 5 | Claude ran with the wrong account / "personal" session sees work config | `~/.zshrc` has `alias claude='claude-work'` — a bare `claude` (even with `CLAUDE_CONFIG_DIR=...` prefixed) silently hits WORK | Use the `claude-work` / `claude-personal` shell functions; verify true MCP/config state by reading the account's `.claude.json` directly | The alias gotcha burned a debugging session pre-2026-07-05; owner-recorded, no in-repo record of the incident — this row is the record (verified 2026-07-12). The `claude-work`/`claude-personal` zshrc functions themselves are listed as external deps in `docs/skill-library-and-infra-handoff.md` |
| 6 | Edited a skill but sessions don't see the change (or see a deleted skill) | Was the session restarted? Skills are repo-scoped and `.claude/skills/` is read at startup. Then `./scripts/skills-status.sh` — any dangling link, or a stale account symlink still shadowing the repo copy? | Edit the real folder under `.claude/skills/`, **restart the session** — discovery is cached; trust the filesystem, not `claude -p` probing. `relink.sh` is only needed for Codex, which mirrors `.claude/codex-skills.txt`. | Repo-scoped rule in **personal-stuff-change-control** |
