...
2. **Pull state from these sources (in order):**
   - Plan files referenced this session (check `$CLAUDE_CONFIG_DIR/plans/` if a plan was mentioned — this is account-specific, e.g. `~/.claude-work/plans/` or `~/.claude-personal/plans/`).
   - TodoWrite / task state — any in-progress or pending tasks.
   ...
   - Memory files written or updated (`$CLAUDE_CONFIG_DIR/projects/<project>/memory/`).
