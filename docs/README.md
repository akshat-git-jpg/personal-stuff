...
- `runbooks/` — step-by-step operating procedures: `memory-maintenance.md` (2026-08-25 — auditing Claude's file-based memory: what memory is for, the four-question test a note must pass, the one-store-per-repo symlink fix, the recurring audit with its commands, and the traps that make an audit go wrong; written as the memory chapter of a repo-maintainer agent's brief), `rotate-gh-token.md`, `windows-custody-setup.md` (2026-08-24 — verifying and repairing the commit-custody machine, pp-work worktrees, auto-commit and auto-land on a Windows laptop; carries a copy-paste prompt that Claude executes end to end).
