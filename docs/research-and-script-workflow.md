...
2. Check if `youtube/yt-research/niches/<slug>/` already exists.
...
- Does `youtube/yt-script/scripts/<slug>/script.md` exist?
...
```
youtube/yt-research/niches/<slug>/
├── niche.md
├── video-ids.md
├── gemini-research.md
├── pricing/
...
Once they answer, write the content to `youtube/yt-research/niches/<slug>/niche.md`. Use the same free-form style as the example below — no rigid template, just clear prose + the tier list.
...
Take whatever they paste, extract the 11-character IDs, and write them as a JSON array to `youtube/yt-research/niches/<slug>/video-ids.md`.
...
Wait for them to paste the content. Once they paste it, write it exactly as-is to `youtube/yt-research/niches/<slug>/gemini-research.md`.
...
>
> - Folder: `youtube/yt-research/niches/<slug>/pricing/`
...
- `youtube/yt-research/niches/<slug>/pricing/` exists
...
>
> `! cd youtube/yt-research && npx ts-node run.ts --niche <slug>`
>
...
The pipeline runs these steps in order: validate → transcripts → pricing → extract → profiles → comparative. All output lands in `youtube/yt-research/niches/<slug>/output/`.
...
Say: **"Synthesize the KB for niche `<slug>`"** — this triggers the instructions in `youtube/yt-research/CLAUDE.md` which Claude will follow to read all Phase 1 outputs and write `knowledge-base.md` + `knowledge-base-compact.md`.
...
Say: **"script for `<slug>`"** — this triggers the instructions in `youtube/yt-script/CLAUDE.md`.

The two-pass workflow:
1. Claude writes `youtube/yt-script/scripts/<slug>/outline.md` and stops.
2. User reviews and approves the outline.
3. Claude writes the full script to `youtube/yt-script/scripts/<slug>/script.md`.
...
| Input | Path |
|-------|------|
| Niche brief (tools, tier list, audience) | `youtube/yt-research/niches/<slug>/niche.md` |
| YouTube video IDs | `youtube/yt-research/niches/<slug>/video-ids.md` |
| Gemini deep research | `youtube/yt-research/niches/<slug>/gemini-research.md` |
| Pricing screenshots | `youtube/yt-research/niches/<slug>/pricing/<tool-name>/` |
| Phase 1 outputs | `youtube/yt-research/niches/<slug>/output/` |
| Knowledge base (full) | `youtube/yt-research/niches/<slug>/output/knowledge-base.md` |
| Knowledge base (compact) | `youtube/yt-research/niches/<slug>/output/knowledge-base-compact.md` |
| Script outline | `youtube/yt-script/scripts/<slug>/outline.md` |
| Final script | `youtube/yt-script/scripts/<slug>/script.md` |
