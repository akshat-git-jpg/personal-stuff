...
// A scene is still "todo" if its index.html carries the scaffold TODO marker.
function isDone(indexPath) {
  try { return !/TODO\(static\)|—\s*TODO/.test(readFileSync(indexPath, 'utf8')); }
  catch { return false; }
}
...
function findScenes() {
  ...
  if (existsSync(exDir)) {
    for (const d of readdirSync(exDir)) {
      if (existsSync(join(exDir, d, 'index.html'))) scenes.push({ group: 'examples', path: `kit/examples/${d}`, name: d, done: true });
    }
  }
  ...
  if (existsSync(vidDir)) {
    for (const v of readdirSync(vidDir)) {
      ...
      for (const s of readdirSync(scDir).sort()) {
        const ip = join(scDir, s, 'index.html');
        if (existsSync(ip)) scenes.push({ group: v, path: `videos/${v}/scenes/${s}`, name: s, done: isDone(ip) });
      }
    }
  }
  return scenes;
...
function gallery() {
  ...
  const vids = scenes.filter((s) => s.group !== 'examples');
  const doneN = vids.filter((s) => s.done).length;
  const tiles = scenes.map((s) => `
    <figure class="tile ${s.done ? 'is-done' : 'is-todo'}">
      <div class="frame"><iframe src="/${s.path}/index.html" scrolling="no" loading="lazy"></iframe>
        <span class="badge">${s.done ? '✓ done' : '· todo'}</span></div>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .empty { color: #8a8a8a; }
  .frame { }
  .badge { position: absolute; top: 8px; right: 8px; z-index: 2; font-size: 11px; letter-spacing: .06em;
    padding: 3px 8px; border-radius: 100px; background: rgba(0,0,0,.6); }
  .is-done .badge { color: #22c55e; } .is-todo .badge { color: #8a8a8a; }
  .is-todo .frame { opacity: .45; border-style: dashed; }
  .sub label { color: #f5f5f5; margin-left: 14px; cursor: pointer; }
  .hide-todo .is-todo { display: none; }
</style></head><body>
  <h1>Devsplainers kit — contact sheet</h1>
  <p class="sub">${doneN}/${vids.length} scenes done
    <label><input type="checkbox" id="play" checked> play motion (loop)</label>
    <label><input type="checkbox" id="onlyDone"> only done</label>
    <label><input type="checkbox" id="live"> live (auto-refresh 15s)</label></p>
  ...
  <script>
    ...
    // Each scene registers a paused GSAP timeline. In "play" mode we loop it
    // (with a hold between loops) so the tile previews the motion; otherwise we
    // freeze it on the final composed frame for inspection.
    const playEl = document.getElementById('play');
    const frames = [...document.querySelectorAll('iframe')];
    function withTl(f, cb, tries = 25) {
      let tl;
      try { tl = f.contentWindow.__timelines && f.contentWindow.__timelines.main; } catch (e) {}
      if (tl) return cb(tl);
      if (tries > 0) setTimeout(() => withTl(f, cb, tries - 1), 120);
    }
    function apply(f) {
      withTl(f, (tl) => {
        if (playEl.checked) { tl.repeat(-1); tl.repeatDelay(1.0); tl.play(0); }
        else { tl.repeat(0); tl.pause(); tl.progress(1); }
      });
    }
    frames.forEach((f) => f.addEventListener('load', () => apply(f)));
    playEl.addEventListener('change', () => frames.forEach(apply));
    // "only done" filter + optional live auto-refresh
    const onlyDone = document.getElementById('onlyDone');
    onlyDone.addEventListener('change', () => document.body.classList.toggle('hide-todo', onlyDone.checked));
    const live = document.getElementById('live');
    let timer = null;
    live.addEventListener('change', () => {
      clearInterval(timer);
      if (live.checked) timer = setInterval(() => location.reload(), 15000);
    });
  </script>
</body></html>`;
}
