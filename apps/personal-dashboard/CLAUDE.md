...
- `APP_PASSWORD` is hashed into the DB on first boot; changing the env var afterwards doesn't change the login (unless the hash in the DB was empty/NULL, in which case a startup self-heal writes the hash of the current `APP_PASSWORD`). Secrets in `.env` (see `.env.example`): `APP_PASSWORD`, `SESSION_SECRET` (required), optional `OPENROUTER_API_KEY`, Google OAuth vars.
