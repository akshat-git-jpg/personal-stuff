...
- **Link generation needs the D1 schema in the LOCAL D1.** `wrangler dev` uses an empty *local* D1, not the remote `clicks-db`, so `/api/generate-links` errors with `no such table: videos` until you seed it once: `npx wrangler d1 execute clicks-db --local --file=../../pipelines/workers/redirector/migrations/0001_init.sql`. Production uses the remote `clicks-db` (already has the tables from the redirector in `pipelines/workers/redirector/` (same repo)).
