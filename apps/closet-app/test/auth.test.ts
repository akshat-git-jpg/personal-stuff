...
const ENV = {
  ...
  PHOTOS: {} as unknown as R2Bucket, // Mock R2
...
describe('lists-app auth flow', () => {
  ...
  it('accepts login with correct password and sets cookie, then verifies me endpoint', async () => {
    ...
    expect(cookie).toContain('closet_session=')

    const match = cookie ? cookie.match(/closet_session=([^;]+)/) : null
    ...
    const res2 = await app.request(
      ...
      {
        method: 'GET',
        headers: { Cookie: `closet_session=${token}` },
      },
      ENV
    )
