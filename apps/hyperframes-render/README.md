...
The Templates tab is a gallery of ready-made cards read live from the TY checkout at `pipelines/video/card-library/`. On the VPS that folder is mounted read-only at `/cards` (set by `CARDS_DIR`) and a cron pulls it every 15 minutes, so adding a template in TY and pushing makes it show up here with no redeploy.
