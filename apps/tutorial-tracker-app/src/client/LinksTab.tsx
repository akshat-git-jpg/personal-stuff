...
type View = "programs" | "tracking-links" | "health";
...
export function LinksTab({ rows: _rows, onSaved }: { rows: BoardRow[]; onSaved: () => void }) {
  ...
  return <div className="max-w-6xl space-y-5 py-4" data-testid="links-tab">
    <div className="flex gap-1 border-b border-border" role="tablist">{([['programs', `Programs${loading ? "" : ` (${programs.length})`}`], ['tracking-links', 'Tracking links'], ['health', 'Health']] as [View, string][]).map(([key, label]) => <button key={key} role="tab" aria-selected={view === key} onClick={() => setView(key)} className={`border-b-2 px-3 py-2 text-sm font-medium transition-colors ${view === key ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}>{label}</button>)}</div>
    {view === "tracking-links" && <section className="space-y-3"><div><h2 className="text-lg font-semibold tracking-tight">Link drift</h2><p className="text-sm text-muted-foreground">Per-video link minting moves here in the next change.</p></div><LinkDriftPanel /></section>}
