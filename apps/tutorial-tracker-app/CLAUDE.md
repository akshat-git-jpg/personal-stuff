...
- **⚠️ Two-source-of-truth trap (2026-08-13).** `control.ts` decides what a card SHOWS; `derive.ts` decides what a role may SEE. The Worker strips everything outside the role's access, so a column shown by one and withheld by the other **silently vanishes from the card** — no lock, no error. Whenever you add a field to a stage's view, grant it in `producerAccess`/`reviewerAccess` too. `test/engine.test.ts` pins `showColumns ⊆ visibleColsForRoles` for every (pipeline × stage × status × worker|reviewer); if it fails, the access layer is the thing to fix. Related rule: a role's columns are visible at **every** status — only editability moves with status.
