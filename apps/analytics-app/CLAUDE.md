...
Per-video analytics dashboard for the `@AgrolloReviews` channel + the `go.agrolloo.com` shortener. **YouTube is the source of truth for the video list** — the dashboard shows the channel's public long-form uploads (Shorts excluded), and each one is enriched with click data from D1 where it exists. A video with no shortener link still shows (0 clicks). Three tabs (`App.tsx`, client-side state): **Clicks** — per video, its **live YouTube view count**, de-duplicated click counts (30d + all-time), and each affiliate/tool link with its own counts (one dense card per video, all rendered upfront, no expand/collapse), with a client-side **Sort by** chip row — Most clicks (default, mirrors the server order) / Newest first / Most views; **Uploads** (`UploadsView.tsx`) — upload-frequency bar chart (week/month toggle) over each video's real YouTube publish date, with preset + custom date ranges and a list of videos uploaded in the selected window (defaults to **Last 7 days** + week buckets; other presets are 30d / 90d / this year / all time); **Rankings** (`RankingsView.tsx`) — per-video keyword rank tracking via a manual YouTube-search check, with rank-over-time line charts (see the Rankings section). Live at `yt-analytics.agrolloo.com`. Worker name `yt-analytics`.
...
Dedup matches `yt-analysis/sync_clicks.py` exactly: a click is keyed by `(slug, ip_hash, ua_hash, clicked_at/3600)`, so one person/hour = 1 click. `analytics.ts` computes all-time + last-30d per slug in one query, then folds links into a per-video tree (video totals = sum of link counts, sorted by all-time desc, ties broken by newest upload — the client can re-sort this in `App.tsx`, see the Clicks tab above). If you change the dedup here, change it in `sync_clicks.py` too, or the dashboard and the sheet will disagree.
