# youtube/analytics-app — YT Analytics dashboard
...
Same stack as `tracker-app/`: Vite + React (client) + Hono on a Cloudflare Worker, SPA served via the `ASSETS` binding.
...
│   └── analytics.ts   # the D1 aggregation (dedup + per-video tree)
├── src/client/        # React SPA (App, Login, api)
...
Binds the redirector's `clicks-db` D1 (id `3415a408-…`, schema owned by `workers/redirector/migrations/`). This app ONLY reads `videos`/`links`/`clicks`. Never INSERT/UPDATE/migrate here — the redirector owns the schema.
...
Two secrets: `APP_PASSWORD`, `SESSION_SECRET` (`wrangler secret put`, mirror in `.dev.vars` for local).
...
npm install
# local D1 starts EMPTY — seed schema first, or use --remote for live data:
npx wrangler d1 execute clicks-db --local --file=../../workers/redirector/migrations/0001_init.sql
