...
- **Review:** `../100-trim-silence/output/<base>.voice.trim.wav` (the full voice) and spot-check
  `../090-plan-corner-render-parts/output/corner-parts/*.wav` (the corner parts).
...
1. Find the timestamp; open `…080-synthesize-voice/output/<base>.work/index.txt` → the chunk id.
...
3. Re-synth only that chunk: `080-synthesize-voice/run.py <input> --only 0042`, then re-run step 090.
