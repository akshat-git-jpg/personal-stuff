...
| CLI tools Claude calls (gmail, sheets, youtube, hostinger, ntfy, rapidapi, yt-claude, cf-email, drive, heygen-web, heygen-api, local-apps-dashboard, flights, flow-queue) | `tooling/cli/` |
| HeyGen renders — which CLI, free vs metered | `tooling/cli/heygen-web/` (free, session) vs `tooling/cli/heygen-api/` (metered, API key) |
