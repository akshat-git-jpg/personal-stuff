...
4. **Execution — default lane is Antigravity** (owner decision 2026-07-06),
   ...
   - Sonnet-subagent executors need no lock (they work here, in YOUR
     worktree) — use them when the captain's brief says so, or for fix-up
     rounds while the lock queue is long.
