...
print("the view setting is left alone")
with tempfile.TemporaryDirectory() as tmp:
    cfg = os.path.join(tmp, "claude.json")
    with open(cfg, "w") as fh:
        json.dump({"fleetViewGroupMode": "directory", "keep": "me"}, fh)
    check("the heal is gone from the module",
          not hasattr(mod, "restore_group_view") and not hasattr(mod, "heal_view"))
    check("no config paths are compiled in", not hasattr(mod, "CONFIGS"))
    check("it says why", "fleetViewGroupMode" in mod.view_setting_note())
    check("it points at the replacement", "pp-agents" in mod.view_setting_note())
    src = open(os.path.join(HERE, "pp-claude-tags")).read()
    check("the source never writes the view setting",
          'fleetViewGroupMode"] =' not in src and "fleetViewGroupMode'] =" not in src)
    check("the docstring records that it is superseded", "SUPERSEDED" in src)
    check("the docstring names the replacement", "pp-agents" in src)
    check("the untouched config is byte-identical",
          json.load(open(cfg)) == {"fleetViewGroupMode": "directory", "keep": "me"})
...
print("the launch agent is retired")
plist = os.path.join(HERE, "com.kbtg.pp-claude-tags.plist")
check("the launch agent file is gone from the repo", not os.path.exists(plist))
