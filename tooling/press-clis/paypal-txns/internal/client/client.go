...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "paypal-txns-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "paypal-txns-pp-cli/1.9")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "paypal-txns-pp-cli auth login or paypal-txns-pp-cli auth set-token <token>")
}
...
func (c *Client) mintClientCredentials(ctx context.Context, clientID, clientSecret string) error {
	...
	// Resolve a relative token path (e.g. "/v1/oauth2/token") against the
	// configured base URL; net/http rejects requests with an empty scheme/host.
	if !strings.HasPrefix(tokenURL, "http://") && !strings.HasPrefix(tokenURL, "https://") {
		base := ""
		if c.Config != nil {
			base = strings.TrimRight(c.Config.BaseURL, "/")
		}
		tokenURL = base + "/" + strings.TrimLeft(tokenURL, "/")
	}
	form := url.Values{
	...
	req.Header.Set("Accept", "application/json")
	// PayPal's /v1/oauth2/token requires HTTP Basic auth (client_id:secret).
