...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PAYPAL_TXNS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "paypal-txns-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("PAYPAL_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.AuthSource = "env:PAYPAL_CLIENT_ID"
	}
	if v := os.Getenv("PAYPAL_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.AuthSource = "env:PAYPAL_CLIENT_SECRET"
	}
	...
	if v := os.Getenv("PAYPAL_TXNS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("PAYPAL_TXNS_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
