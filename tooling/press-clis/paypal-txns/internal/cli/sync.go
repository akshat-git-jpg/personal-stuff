...
Resource scoping:
  ...
  paypal-txns-pp-cli sync

  # Sync specific resources only
  paypal-txns-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  paypal-txns-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  paypal-txns-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  paypal-txns-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  paypal-txns-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/paypal-txns-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"reporting": "/v1/reporting/balances",
	}
