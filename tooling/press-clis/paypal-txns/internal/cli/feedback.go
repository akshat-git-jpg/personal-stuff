...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "paypal-txns-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PAYPAL_TXNS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PAYPAL_TXNS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "paypal-txns-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/paypal-txns-pp-cli/feedback.jsonl.
When ` + "`PAYPAL_TXNS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PAYPAL_TXNS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  paypal-txns-pp-cli feedback list
  paypal-txns-pp-cli feedback list --limit 5
  paypal-txns-pp-cli feedback list --json`,
