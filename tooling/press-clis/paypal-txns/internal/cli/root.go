...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "paypal-txns-pp-cli",
		Short: `Paypal Txns CLI — Read-only PayPal income reporting that handles the 31-day window limit for you - pull months of transactions and a mont…`,
		Long: `Paypal Txns CLI — Read-only PayPal income reporting that handles the 31-day window limit for you - pull months of transactions and a mont…

Highlights (not in the official API docs):
  • income   Total money received from PayPal grouped by month and currency over any date range.
...
Health check: run 'paypal-txns-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("paypal-txns-pp-cli {{ .Version }}\n")

	rootCmd.PersistentFlags().BoolVar(&flags.asJSON, "json", false, "Output as JSON")
	rootCmd.PersistentFlags().BoolVar(&flags.table, "table", false, "Force the human-readable table even when output is piped or redirected")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'paypal-txns-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelIncomeCmd(flags))
