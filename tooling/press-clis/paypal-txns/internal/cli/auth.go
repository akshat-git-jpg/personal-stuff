...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Transaction Search",
...
Mint an OAuth2 bearer token via POST /v1/oauth2/token with
grant_type=client_credentials and cache it on disk. Subsequent commands
auto-refresh the token before expiry.

Credentials default to PAYPAL_CLIENT_ID (Client ID) and PAYPAL_CLIENT_SECRET (Client Secret) environment variables.
`),
		Example: strings.Trim(`
  # Use env vars
  paypal-txns-pp-cli auth login

  # Explicit credentials
  paypal-txns-pp-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or PAYPAL_CLIENT_ID/PAYPAL_CLIENT_SECRET)"))
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $PAYPAL_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $PAYPAL_CLIENT_SECRET")
...
func resolveClientCredentialsScope() string {
	if scope := os.Getenv("PAYPAL_TXNS_OAUTH_SCOPE"); scope != "" {
		return scope
	}
	return "https://uri.paypal.com/services/reporting/balances/read https://uri.paypal.com/services/reporting/search/read"
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  paypal-txns-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export PAYPAL_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export PAYPAL_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  paypal-txns-pp-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  paypal-txns-pp-cli auth set-token <bearer-jwt>",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  paypal-txns-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("PAYPAL_CLIENT_ID") != "" {
				envStillSet = "PAYPAL_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("PAYPAL_CLIENT_SECRET") != "" {
				envStillSet = "PAYPAL_CLIENT_SECRET"
			}
