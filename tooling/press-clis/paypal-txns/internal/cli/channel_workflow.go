...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  paypal-txns-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  paypal-txns-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  paypal-txns-pp-cli workflow status

  # Show status as JSON
  paypal-txns-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("paypal-txns-pp-cli")
			}
