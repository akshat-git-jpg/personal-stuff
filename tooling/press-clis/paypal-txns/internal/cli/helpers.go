...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'paypal-txns-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: paypal-txns-pp-cli auth set-token <token>"+
			"\n      or: export PAYPAL_CLIENT_ID=<your-token>"+
			"\n      See API docs: https://developer.paypal.com/docs/api/reporting/v1/"+
			"\n      Run 'paypal-txns-pp-cli doctor' to check auth status.", err))
