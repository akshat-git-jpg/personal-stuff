...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  paypal-txns-pp-cli doctor
  paypal-txns-pp-cli doctor --json
  paypal-txns-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export PAYPAL_CLIENT_ID=<your-key>"
					report["auth_docs_url"] = "https://developer.paypal.com/docs/api/reporting/v1/"
				} else {
					authConfigured = true
			...
			if os.Getenv("PAYPAL_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "PAYPAL_CLIENT_ID")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PAYPAL_CLIENT_ID")
			}
			if os.Getenv("PAYPAL_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "PAYPAL_CLIENT_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PAYPAL_CLIENT_SECRET")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "paypal-txns-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("paypal-txns-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'paypal-txns-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'paypal-txns-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'paypal-txns-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'paypal-txns-pp-cli sync' to refresh."
	}
	return report
}
