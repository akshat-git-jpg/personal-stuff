# PayPal Transaction Search Printed CLI Agent Guide

This directory is a generated `paypal-txns-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
paypal-txns-pp-cli doctor --json
paypal-txns-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
paypal-txns-pp-cli which "<capability>" --json
paypal-txns-pp-cli <command> --help
```
...
```bash
paypal-txns-pp-cli <command> --agent
```
...
```bash
paypal-txns-pp-cli <command> --help
paypal-txns-pp-cli <command> --dry-run --agent
```
