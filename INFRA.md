...
- **trip-planner** — `trips.agrolloo.com` — trip pin map PWA with a Plan view (days, bookings, Drive doc links). No read gate. Binding: `TRIPS_KV` (KV `8c1613e4d24d43e78fa81c54aa385bc4`). Secret: `ADMIN_TOKEN` (write-only, used by `pp-trip`). Source still on branch `work/trip-planner`.
